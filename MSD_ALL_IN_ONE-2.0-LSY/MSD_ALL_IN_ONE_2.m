...
% 定义粒子轨迹文件路径
filePath = 'Trajectory Data\combined_trajectories_for_msd (20mM H2O2)-(154-750_2651-2950_800-1099 frames).csv';
...
if ~isempty(tracks)
    ...
    for i = 1:length(tracks)
        ...
        xlabel("\mu m^2",'Interpreter','latex');
...
%%
% display MSD cuvers of each particle 
figure
MSD.plotMSD;
...
MSD.labelPlotTracks;
%%
% plot the log-log figure of the corresponding MSD-Delta t figure 
MMSD = MSD.getMeanMSD;
t = MMSD(:,1);
x = MMSD(:,2);
...
ylabel("$\log\langle \Delta L^2\rangle$",'Interpreter','latex');
%%
% plot the local slop of the log-log figure for characterizing the
% ballistic regime and diffusive regime

% Visualization of linear fitting of the diffusive regime
figure
MSD.plotMeanMSD(gca, true)
MMSD = MSD.getMeanMSD;
t = MMSD(:,1);
...
%%
% Visualization of quadratic fitting of the ballistic regime
SPACE_UNITS = 'µm';
TIME_UNITS = 's';
...
% export the result of linear fitting 
...
%% 
%export the result of quadratic fitting 
...
%   time_limit     - The upper time limit for exporting and plotting the full dataset.
    ...
    ft = fittype('a*x + c*x^2');
    [fo, gof] = fit(t_fit, msd_mean_fit, ft, 'Weights', 1./msd_std_fit.^2, 'StartPoint', [0 0]);
...
if exist('MMSD', 'var') && ~isempty(MMSD)
    ...
    export_and_plot_partial_quadratic_fit(MMSD, 0.1, 2.0); % Fit on first 20%
else
