...
if ~isempty(tracks)
    ...
    export_and_plot_final_data(MSD, MMSD, fit_percentage, 2.0);
else
...
%%
% manual fitting on selected time interval 
% Get the averaged MSD
MMSD = MSD.getMeanMSD;
t = MMSD(:,1);
y = MMSD(:,2);
w = 1 ./ MMSD(:,3);   % weights (inverse std)

% Select custom interval, e.g. between 2 and 3 seconds
idx = (t >= 2) & (t <= 3);
...
% Perform weighted linear fit (poly1 = y = p1*x + p2)
ft = fittype('poly1');
[fo, gof] = fit(t_fit, y_fit, ft, 'Weights', w_fit);

% Extract diffusion coefficient (for Brownian regime)
D = fo.p1 / (2 * 2);

disp(['Custom fit interval: D = ', num2str(D)]);
