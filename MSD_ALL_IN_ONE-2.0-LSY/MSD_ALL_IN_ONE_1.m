...
%%
% Linear fitting
% Plot the mean MSD with error bar
figure
MSD.plotMeanMSD(gca, true)
MMSD = MSD.getMeanMSD;
t = MMSD(:,1);
x = MMSD(:,2);
...
% Curve fitting for the first 20% data
[fo, gof] = MSD.fitMeanMSD(0.2);
plot(fo)
MSD.labelPlotMSD;
legend off
...
[fo, gof] = fit(t, x, ft, 'Weights', 1./std_msd, 'StartPoint', [0 0]);
...
%%
% --- Corrected and Robust Quadratic Fitting ---
...
% --- ROBUST WEIGHT CALCULATION (START) ---
% The original line below is a good start but not enough
% std_msd(1) = std_msd(2); 

% 1. Calculate weights
weights = 1./std_msd.^2; % It's more common to use variance (std^2) for weights

% 2. Clean the weights: Find any invalid numbers (NaN, Inf) and set them to 0.
% A weight of 0 means this data point will be ignored by the fit.
weights(~isfinite(weights)) = 0;

% 3. Check for any remaining negative weights (highly unlikely but safe to check)
weights(weights < 0) = 0;
% --- ROBUST WEIGHT CALCULATION (END) ---

ft = fittype('a*x + c*x^2');

% Use the cleaned 'weights' vector in the fit function
[fo, gof] = fit(t, x, ft, 'Weights', weights, 'StartPoint', [0 0]);
...
if ~isempty(tracks)
    ...
    export_and_plot_final_data(MSD, MMSD, 1.5);
    export_and_plot_final_data(MSD, MMSD, 2.0);
else
...
function export_and_plot_final_data(MSD, MMSD, time_limit)
    
    fprintf('\n--- Processing for time limit: %.1fs ---\n', time_limit);
    
    % --- NEW: Perform a specific fit for each time limit ---
    if time_limit == 1.5
        fit_fraction = 0.15;
    elseif time_limit == 2.0
        fit_fraction = 0.2;
    else
        fit_fraction = 0.15; % A sensible default
    end
    fprintf('Performing linear fit using the first %.0f%% of the MSD curve...\n', fit_fraction * 100);
    [fo, gof] = MSD.fitMeanMSD(fit_fraction);
...
%%
function export_and_plot_quadratic_fit(MMSD, time_limit)
% This function performs a quadratic fit on the mean MSD data,
% exports the results to a CSV file, and generates a publication-ready plot.
%
% INPUTS:
%   MMSD       - The Mean MSD matrix from msdanalyzer [time, msd, std, n].
%   time_limit - The upper time limit (e.g., 1.5 or 2.0) for exporting and plotting.

    fprintf('\n--- Processing Quadratic Fit for time limit: %.1fs ---\n', time_limit);
    ...
    % --- 2. Perform Quadratic Fitting ---
    % Fit the entire MSD curve to get the best overall parameters.
    % The model is MSD(t) = a*t + c*t^2, where D=a/4 and V=sqrt(c)
    fprintf('Performing quadratic fit (a*t + c*t^2) on the full MSD curve...\n');
    ft = fittype('a*x + c*x^2');
    [fo, gof] = fit(t, msd_mean, ft, 'Weights', 1./msd_std, 'StartPoint', [0 0]);
    ...
    % --- 4. Filter and Prepare Data for Export ---
    disp('Filtering and preparing final data for export...');
    export_indices = MMSD(:, 1) <= time_limit;
    ...
    output_csv_path = sprintf('MSD Data/MSD_Quadratic_Fit_Filtered_%.1fs.csv', time_limit);
    writetable(export_table, output_csv_path);
    fprintf('Successfully saved quadratic fit data to %s\n', output_csv_path);

    % --- 6. Create Final Publication Plot ---
    disp('Creating final publication plot for quadratic fit...');
    figure; 
    hold on;
    ...
    % Plot the quadratic fit curve up to the time limit
    % h_fit = plot(fo);
    % set(h_fit, 'Color', 'k', 'LineWidth', 1.5);
    
    % Configure plot aesthetics
    title_text = sprintf('MSD Quadratic Fit (D = %.3f \\mum^2/s, V = %.3f \\mum/s)', diffusion_coeff, velocity);
    title(title_text);
    xlabel('Time (s)');
    ylabel('MSD (\mum^2)');
    grid on;
...
% --- Section 9. Quadratic Fit Analysis & Export ---
disp('--- Section 9: Quadratic Fit Analysis on Filtered Data ---');

if exist('MMSD', 'var') && ~isempty(MMSD)
    % Call the new function for the two required time limits
    export_and_plot_quadratic_fit(MMSD, 2.0);
else
    disp('MMSD data not found. Please run the MSD calculation first.');
end
...
if exist('MMSD', 'var') && ~isempty(MMSD)
    ...
    export_and_plot_partial_quadratic_fit(MMSD, 1, 2.0); % Fit on first 20%
else
